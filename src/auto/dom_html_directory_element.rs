...
impl<O: IsA<DOMHTMLDirectoryElement>> DOMHTMLDirectoryElementExt for O {
    fn get_compact(&self) -> bool {
        unsafe {
            from_glib(
                webkit2_webextension_sys::webkit_dom_html_directory_element_get_compact(
    ...
    fn set_compact(&self, value: bool) {
        unsafe {
            webkit2_webextension_sys::webkit_dom_html_directory_element_set_compact(
                self.as_ref().to_glib_none().0,
                value.to_glib(),
            );
        }
    ...
    fn connect_property_compact_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_compact_trampoline<P, F: Fn(&P) + 'static>(
            this: *mut webkit2_webextension_sys::WebKitDOMHTMLDirectoryElement,
