...
impl<O: IsA<DOMHTMLParagraphElement>> DOMHTMLParagraphElementExt for O {
    fn get_align(&self) -> Option<GString> {
        unsafe {
            from_glib_full(
                webkit2_webextension_sys::webkit_dom_html_paragraph_element_get_align(
    ...
    fn set_align(&self, value: &str) {
        unsafe {
            webkit2_webextension_sys::webkit_dom_html_paragraph_element_set_align(
                self.as_ref().to_glib_none().0,
    ...
    fn connect_property_align_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_align_trampoline<P, F: Fn(&P) + 'static>(
            this: *mut webkit2_webextension_sys::WebKitDOMHTMLParagraphElement,
