...
impl DOMDOMImplementation {
    ...
    pub fn create_document<'a, T: Into<Option<&'a str>>>(&self, namespaceURI: T, qualifiedName: &str, doctype: Option<&DOMDocumentType>) -> Result<DOMDocument, Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::webkit_dom_dom_implementation_create_document(self.to_glib_none().0, namespaceURI.into().to_glib_none().0, qualifiedName.to_glib_none().0, doctype.to_glib_none().0, &mut error);
