...
pub trait DOMElementExt {
    ...
    fn set_attribute_ns<'a, T: Into<Option<&'a str>>>(&self, namespaceURI: T, qualifiedName: &str, value: &str) -> Result<(), Error>;
...
impl<O: IsA<DOMElement> + IsA<Object>> DOMElementExt for O {
    fn blur(&self) {
        unsafe {
    ...
    fn set_attribute_ns<'a, T: Into<Option<&'a str>>>(&self, namespaceURI: T, qualifiedName: &str, value: &str) -> Result<(), Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let _ = ffi::webkit_dom_element_set_attribute_ns(self.to_glib_none().0, namespaceURI.into().to_glib_none().0, qualifiedName.to_glib_none().0, value.to_glib_none().0, &mut error);
            if error.is_null() { Ok(()) } else { Err(from_glib_full(error)) }
        }
    }
