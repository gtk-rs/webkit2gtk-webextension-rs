...
pub trait DOMNodeExt {
    ...
    fn get_property_tree_root(&self) -> Option<DOMNode>;
}

impl<O: IsA<DOMNode> + IsA<Object>> DOMNodeExt for O {
    fn append_child<T: IsA<DOMNode>>(&self, newChild: &T) -> Result<DOMNode, Error> {
        unsafe {
    ...
    fn get_property_tree_root(&self) -> Option<DOMNode> {
        ...
        unsafe {
            gobject_ffi::g_object_get_property(self.to_glib_none().0, "tree-root".to_glib_none().0, value.to_glib_none_mut().0);
        }
        value.get()
    }
}
