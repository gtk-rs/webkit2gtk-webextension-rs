...
impl WebPage {
    ...
    pub fn connect_send_request<F: Fn(&WebPage, &URIRequest, &URIResponse) -> bool + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&WebPage, &URIRequest, &URIResponse) -> bool + 'static>> = Box_::new(Box_::new(f));
