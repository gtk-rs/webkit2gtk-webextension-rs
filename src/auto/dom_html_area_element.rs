...
impl DOMHTMLAreaElement {
    ...
    pub fn get_property_rel(&self) -> Option<String> {
        let mut value = Value::from(None::<&str>);
        unsafe {
            gobject_ffi::g_object_get_property(self.to_glib_none().0, "rel".to_glib_none().0, value.to_glib_none_mut().0);
        }
        value.get()
    }

    pub fn set_property_rel(&self, rel: Option<&str>) {
        unsafe {
            gobject_ffi::g_object_set_property(self.to_glib_none().0, "rel".to_glib_none().0, Value::from(rel).to_glib_none().0);
        }
    }
}
